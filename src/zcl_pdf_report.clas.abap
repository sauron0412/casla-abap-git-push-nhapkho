...
*
      ...
      PRIVATE SECTION.
        TYPES: BEGIN OF ty_header_json,
                 ...
                 numberofreservation  TYPE string,
                 ...
                 storagelocation      TYPE lgort_d,        " Mã kho
                 storagelocation_text TYPE c LENGTH 60,    " Tên kho
                 companyaddress  TYPE c LENGTH 200,   "Địa chỉ công ty
...
*    rsnum
        ...
        CLASS-DATA:
          ...
          ir_MaterialDocument TYPE zcl_goods_receipt_form_query=>tt_ranges.
...
*        DATA: ls_header TYPE I_MATERIALDOCUMENTHEADER_2,  " cấu trúc của CDS view
*          lv_delivery_doc TYPE <Delivery Document on Header>.
*        SELECT SINGLE
*        Delivery Document on Header
*        FROM i_materialdocumentitem_2
*        WHERE materialdocumentitem = @'5000000802'.
    ...
    gs_header = VALUE ty_header_json(
      ...
      vendorname           = ls_first_row-suppliername   " GFI
    ...
    IF sy-subrc = 0.
      ...
      " Nếu không có DeliveryDocument nhưng có Reservation => phiếu nhập kho từ Reservation
      ...
      " Nếu không có cả hai
      ELSE.
        gs_header-numberofreservation = 'Không xác định nguồn nhập kho'.
      ENDIF.
    ...
    " Sau khi lấy ra lv_storagelocation_name ở bước trên
      gs_header-storagelocation     = lv_storagelocation.
...
*gs_header-companyname    = ls_companycode-companycodename.
*gs_header-companyaddress = |{ ls_companycode-companycodeaddr }|.
...
*
*        lv_total_amount = amount_in_words->read_amount(
*          EXPORTING
...
*
    ...
    |<Content3></Content3>| &&
    |<Content4>{ gs_header-storagelocation  }</Content4>| &&
    |</HeaderSection>| &&
